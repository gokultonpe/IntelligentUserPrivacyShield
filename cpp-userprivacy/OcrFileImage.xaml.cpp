...
/*
task<void> OcrFileImage::StoreImage(StorageFile^ file)
{
	return create_task(file->OpenAsync(FileAccessMode::Read)).then([this](IRandomAccessStream^ stream)
		{
			return BitmapDecoder::CreateAsync(stream);
...
void OcrFileImage::UpdateWordBoxTransform()
{
	...
	//scaleTrasform->ScaleX = PreviewImage->ActualWidth;
	//scaleTrasform->ScaleY = PreviewImage->ActualHeight;
...
void OcrFileImage::ExtractButton_Tapped(Platform::Object^ sender, TappedRoutedEventArgs^ e)
{
    ...
    if (ocrEngine != nullptr)
    {
        ...
        create_task(recognizeOp).then([this, ocrEngine](OcrResult^ ocrResult)
        {
			...
			//printf("Recognized test = %s", ocrResult->Text);
			rootPage->NotifyUser(
				"Recognized text:" + ocrResult->Text,
				NotifyType::StatusMessage);
            ...
            for (const auto& line : ocrResult->Lines)
            {
				...
				unsigned int i = 0;
				bool bUserPrivacyTagDetectedInPreviousPass = false;
				...
				for (const auto& word : line->Words)
                {
					...
					// TBD: identify sword sequences instead of single words.
					//const auto& NextWord = line->Words->GetAt(i+1);

					//const auto& nextword = line->Words->GetAt( + 1)
                     //Keep references to word boxes.
                     //wordBoxes.push_back(wordBoxOverlay);

					if (bUserPrivacyTagDetectedInPreviousPass)
					{

					}
					if (word->Text == "Password" ||
						word->Text == "password" || 
						word->Text == "PASSWORD" || 
						word->Text == "Password?" || 
						word->Text == "Password:" ||
						...
						word->Text == "Birth" ||
						word->Text == "Address" ||
						bUserPrivacyTagDetectedInPreviousPass)
					{
						
						if (bUserPrivacyTagDetectedInPreviousPass == false)
						{
							bUserPrivacyTagDetectedInPreviousPass = true;
						}
						...
						// fill colour
						//if (word->Text == "Password" || word->Text == "password")
						//{
			...
			rootPage->NotifyUser(
					"bitmap->PixelHeight" + bitmap->PixelHeight,
					NotifyType::StatusMessage);

			rootPage->NotifyUser(
				"bitmap->PixelWidth" + bitmap->PixelWidth,
				NotifyType::StatusMessage);
